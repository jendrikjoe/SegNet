...
from Data.Datasets import CamVid, Kitti, CityScapes
...
if __name__ == '__main__':
    ...
    if 0:
        weights = inverstigateDatasets([dataset], classDict)
        with open('normWeights.pickle', 'wb') as handle:
            pickle.dump(weights, handle, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        with open('normWeights.pickle', 'rb') as handle:
            weights = pickle.load(handle)
        plt.plot(range(dataset.numberOfClasses), weights)
        plt.xticks(range(dataset.numberOfClasses), 
                   [dataset.id2label[i].name for i in range(dataset.numberOfClasses)],
                   rotation=90)
        plt.subplots_adjust(bottom=0.3)

        plt.show()
    weights = np.array([5e6/val if val > 5e6 else 1 for val in weights])
    plt.plot(range(dataset.numberOfClasses), weights)
    plt.xticks(range(dataset.numberOfClasses), 
               [dataset.id2label[i].name for i in range(dataset.numberOfClasses)],
               rotation=90)    
    ...
    learningRate = tf.train.exponential_decay(
                    ...
                    (1.-0.0005),                # Decay rate.
                    staircase=True)
    segNet = SegNet(inputShape, numberOfClasses, learningRate, startEpoch*batchPerEpoch)
    ...
    for i in np.arange(startEpoch, numEpochs,1):
        if i == 0:
            ...
            print(res.shape)
            print(res.dtype)
            mpimg.imsave('../images/initialImage.png', cv2.addWeighted(res, 0.5, imageTemp, 0.5,0))
        ...
        res = dataset.convertTargetToImage(np.reshape(segNet.eval(image,sess),
                    (inputShape[1], inputShape[2])))
        imageTemp = (255.*image[0]).astype('uint8')
        mpimg.imsave('../images/Epoch%d.png'%i, cv2.addWeighted(res, 0.5, imageTemp, 0.5, 0))
        checkpointFile = os.path.join('./ckpt', 'trainCkpt.ckpt')            

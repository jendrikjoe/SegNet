...
class SegNetTrainer(Trainer):
    ...
    def train_net(self, x, target):
        ...
        print(self.base.number_of_classes * self.loss_criterion(output, labels_one_hot).mean())
        ...
        acc = torch.mean(acc).data.cpu().numpy()
        loss.backward()
        self.optimizer.step()
    ...
    def auto_train(self, epochs, datasets, data_probs, batches_per_epoch,
        ...
        for i in np.arange(epochs):
            ...
            for j in range(batches_per_epoch):
                ...
                if np.isnan(tmp_ce) or tmp_acc == 0: quit()
                batch_ce += tmp_ce
                train_acc += tmp_acc
            ...
            if test_ce < (cross_entropy_comp - .001):
                torch.save(self.base.state_dict(), './ckpt/bestModel.ckpt')
                count = 0
    ...
    def val(self, x, target):
        ...
        with torch.no_grad():
            ...
            labels_one_hot = self.one_hot_encode(target)
            loss = (self.base.number_of_classes *
                    (self.loss_criterion(output, labels_one_hot) * (target != 0).float()).mean())
            acc = (torch.argmax(output, dim=3) == torch.argmax(labels_one_hot, dim=3)).float()
            acc = torch.mean(acc).data.cpu().numpy()
        return loss.data.cpu().numpy(), acc
    ...
    def set_weights(self, weight_file):
        ...
        for i in range(len(self.base.convs)):
            ...
            state_dict['conv%d.weight' % i] = torch.from_numpy(weights['kernel']).cuda()
            state_dict['bn%d.bias' % i] = torch.from_numpy(weights['bias']).cuda()
            state_dict['bn%d.weight' % i] = torch.from_numpy(weights['gamma']).cuda()
            state_dict['bn%d.running_mean' % i] = torch.from_numpy(weights['movingMean']).cuda()
            state_dict['bn%d.running_var' % i] = torch.from_numpy(weights['movingVariance']).cuda()

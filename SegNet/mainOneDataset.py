...
if __name__ == '__main__':
    ...
    seg_net = SegNet(number_of_classes, dropProb=.2)
    device = 'cuda:0'
    seg_net.to(device)
    ...
    trainer = SegNetTrainer(seg_net, optim.Adam, 1e-3, weight_decay=1e-2,
                            scheduler_class=LambdaLR, log_path='./logs/', log_prefix='',
                            weights=Variable(torch.from_numpy(weights)).float().cuda(),

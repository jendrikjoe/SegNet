...
def convertHSVToRGB(img):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            ...
            if hi == 0 or hi == 6:
                temp[i, j] = [img[i, j, 2], t, p]
            elif hi == 1:
                temp[i, j] = [q, img[i, j, 2], p]
            elif hi == 2:
                temp[i, j] = [p, img[i, j, 2], t]
            elif hi == 3:
                temp[i, j] = [p, q, img[i, j, 2]]
            elif hi == 4:
                temp[i, j] = [t, p, img[i, j, 2]]
            else:
                temp[i, j] = [img[i, j, 2], p, q]
    return temp

def augmentImageAndLabel(image, label, depth=None):
    ...
    if depth is not None:
        depth = shiftImg(depth, shiftHor, shiftVer)
...
def shiftImg(arr, horizontal, vertical):
    if len(arr.shape) == 2:
        if(horizontal > 0):
            arr = np.concatenate((arr[horizontal:,:],np.zeros((horizontal,arr.shape[1]))), axis=0)
        elif(horizontal < 0):
            arr = np.concatenate((np.zeros((np.abs(horizontal),arr.shape[1])), arr[:horizontal,:]), axis=0)
        if(vertical > 0):
            arr = np.concatenate((arr[:,vertical:],np.zeros((arr.shape[0],vertical))), axis=1)
        elif(vertical < 0):
            arr = np.concatenate((np.zeros((arr.shape[0], np.abs(vertical))), arr[:,:vertical]), axis=1)
   
    else:
        if(horizontal > 0):
            arr = np.concatenate((arr[horizontal:], np.zeros((horizontal, arr.shape[1], arr.shape[2]))), axis=0)
        elif(horizontal < 0):
            arr = np.concatenate((np.zeros((np.abs(horizontal), arr.shape[1], arr.shape[2])), arr[:horizontal]), axis=0)
        if(vertical > 0):
            arr = np.concatenate((arr[:, vertical:], np.zeros((arr.shape[0], vertical, arr.shape[2]))), axis=1)
        elif(vertical < 0):
            arr = np.concatenate((np.zeros((arr.shape[0], np.abs(vertical), arr.shape[2])), arr[:,:vertical]), axis=1)
    return arr
